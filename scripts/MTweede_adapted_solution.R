...
MTweedieTests <-  
  function(N,M,sig){ 
    ...
    cl<- makeCluster(Cores) #make clusters
    registerDoParallel(cl)
    clusterExport(cl, c("N","sig", "simTweedieTest","rtweedie"),envir=environment()) #export function to clusters
    results<-parLapply(cl, 1:M,function(i){#using parallel processing to run the simulation
      replicate(M,simTweedieTest(N))<sig #running this simulation M times
    }) 
